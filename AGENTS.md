...
- Use Yarn and keep `yarn.lock` authoritative.
...
- Build both package targets: `yarn build`
- Run tests: `yarn test`
- Run tests with CI coverage: `yarn test --coverage --run`
- Regenerate JSON and TypeScript config files from YAML: `yarn yaml-to-json`
- Create YAML definitions from a raw export: `yarn prepare-yaml <fifa-version>`
...
- When changing a table definition, edit the corresponding file under `config/` and run `yarn yaml-to-json`; commit the related generated changes under `src/config/` and `src/shared/config.ts`.
...
For code or data changes, run `yarn build` and `yarn test --coverage --run`. For workflow-only or documentation-only changes, validate the affected YAML or Markdown and run the full suite when practical.
